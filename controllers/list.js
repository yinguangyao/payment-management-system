...
exports.delete= function(req, res) {
  ...
  if (id) {
  	List.findById(id,function(err,list){
  		...
  		Comment.remove({_id:{$in:list.comment}})
  		.exec(function(err){
  		if(err){return console.log(err);}
...
exports.view=function(req,res){
	...
	Category.fetch(function(err,categories){
		...
		List.findOne({_id:id})
		.populate({path:"category comment"})
		.exec(function(err,list){
		if(err){return console.log(err);}
		Comment.find({list:list._id})
		.populate({path:"from reply.from reply.to"})
		.sort("meta.updateAt")
